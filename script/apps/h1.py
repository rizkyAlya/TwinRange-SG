...
CYCLE_INTERVAL_S = float(os.environ.get("FIELD_LOOP_INTERVAL_S", "1"))
...
def sync_breakers_from_modbus():
    ...
    cmd_id = int(context[SLAVE_ID].getValues(FX_HR, CMD_ID_ADDR, count=1)[0])
    origin_cycle = int(context[SLAVE_ID].getValues(FX_HR, ORIGIN_CYCLE_ADDR, count=1)[0])
    log_control_plane(
        "h1",
        {
            "cmd_id": cmd_id,
            ...
            "breaker_DT": breaker_state(breaker_status),
        },
    )
    ...
    for bus in range(1, NUM_BUS + 1):
        ...
        log_data_plane(
            "h1",
            {
                ...
                "breaker_actual": breaker_status[bus],
            },
        )
        brk_note = (
            "n/a"
...
def main_loop(net, bus_nums, loads, switches_by_bus):
    ...
    publish_measurements(net, bus_nums, next_cycle_id())
    ...
    while True:
        ...
        remain = CYCLE_INTERVAL_S - elapsed
        if remain > 0:
            time.sleep(remain)
